...
def solveODE(m, rho_air_value, y_points, diam, height, g0):
    ...
    res = solve_ivp(lambda t, y: fun(t, y, m=m, d=diam, rho=float(rho_air_value/1000), g=g0), [y_points[0], y_points[-1]], y0=np.array([0,0]), t_eval=y_points,events=tla)
    y,v=res.y

    #Izračun pospeška
    a=fun(y_points,res.y,m=m, d=diam, rho=float(rho_air_value/1000),g=g0)[1]
